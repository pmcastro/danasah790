...
void uci() {
	...
	printf("option name UCI_Elo type spin default 1500 min 0 max 2500\n");
	printf("option name TYPE_Elo type combo default FIDE var FIDE var USCF var ACTIVE var CCRL\n");
	...
	printf("option name Diff engine type spin default 50 min -400 max 400\n");
...
#if defined (UNDER_ELO)
	printf("info Default Elo engine : 1500\n");
#endif
...
void ParseGoCommand() {
	...
	quedan = 25;									/* si no hay número de movimientos para el control imaginamos que quedan 40 */
	...
	/* si no recibimos el comando ponder hacemos una búsqueda con las opciones de tiempo del comando go */
		...
		else if (inc > 0 && tiempo_total < 3*inc) {
...
/* las siguientes opciones son solo para la version limitada en fuerza */
	...
	/* indica que clase de oponente es: engine (motor), chesscomputer (maquina dedicada de ajedrez) o human (humano) */
	else if (!strcmp(Name, "TYPE_Elo")) {
