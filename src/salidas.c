...
void print_pv(int depth, int score) {
	...
	if (nodos > noise && protocolo == XBOARD) {
        ...
        /*if (strcmp(limitstrength, "true") == 0) {
			...
			else if (strcmp (elo_type, "chesscomputer") == 0)
				printf(" [%d ELO chesscomputer]", elo + diff_engine + diff_chesscomputer_engine);
			else
				printf(" [%d ELO human]", elo + diff_engine + diff_chesscomputer_engine + diff_human_chesscomputer);
		}*/
#endif
	...
	if (protocolo == UCI) {
		...
		printf("info depth %d seldepth %d time %d nodes %d ", depth, seldepth, tiempousado, nodos);
		/* no calculamos nps para los primeros plys ya que no suele dar una buena media */
		if (tiempousado > 16)
#if defined (UNDER_ELO)
			printf("nps %d ", (int)nps_elo);
#else
